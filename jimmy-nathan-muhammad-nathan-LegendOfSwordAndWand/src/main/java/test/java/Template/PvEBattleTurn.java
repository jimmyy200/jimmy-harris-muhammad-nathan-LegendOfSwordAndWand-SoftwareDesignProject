...
// Concrete implementation of the battle turn template for PvE battles
public class PvEBattleTurn extends BattleTurnTemplate {
    ...
    protected void determineTurnOrder(List<Hero> party) {
        // Sort by highest level first, then by highest attack
    ...
    protected void executeMobActions(List<Hero> party, List<Mob> mobs) {
        ...
        for (Mob mob : mobs) {
            ...
            // action 1 = defend (no action), action 2 = wait (no action for mobs)
        }
    }

    @Override
    protected void checkBattleResult(List<Hero> party, List<Mob> mobs) {
        // Checked externally by GamePanel
    }
